def rdm_compare(rdms, models, comp=None, plot=None):
    ...
    if comp is None or comp == 'spearman':
        ...
        for index, part_rdm in enumerate(target_rdms_trans):
            list_cor_rdm[index], list_p[index] = spearmanr(sym_matrix_to_vec(part_rdm, discard_diagonal=True), sym_matrix_to_vec(rdm_avg.to_numpy(), discard_diagonal=True))
        ...
        for index, part_rdm in enumerate(target_rdms):
            list_cor_rdm[index], list_p[index] = kendalltau(sym_matrix_to_vec(part_rdm, discard_diagonal=True), sym_matrix_to_vec(rdm_avg.to_numpy(), discard_diagonal=True))
        ...
        for index, part_rdm in enumerate(target_rdms_trans):
            list_cor_rdm[index], list_p[index] = pearsonr(sym_matrix_to_vec(part_rdm, discard_diagonal=True), sym_matrix_to_vec(rdm_avg.to_numpy(), discard_diagonal=True))
    ...
    snd_rdms = list()
    snd_rdms.append(sym_matrix_to_vec(rdm_avg.to_numpy(), discard_diagonal=True))
    for mod_rdm in models:
        snd_rdms.append(sym_matrix_to_vec(mod_rdm.to_numpy(), discard_diagonal=True))
    ...
    if comp is None or comp == 'spearman':
        for index, model_rdm in enumerate(dict_models['rdm']):
            for i, sub_rdm in enumerate(target_rdms_trans):
                list_cor_models.append(spearmanr(sym_matrix_to_vec(sub_rdm, discard_diagonal=True), sym_matrix_to_vec(model_rdm.to_numpy(), discard_diagonal=True)).correlation)
        ...
        for index, model_rdm in enumerate(dict_models['rdm']):
            for i, sub_rdm in enumerate(target_rdms_trans):
                list_cor_models.append(kendalltau(sym_matrix_to_vec(sub_rdm, discard_diagonal=True), rankdata(sym_matrix_to_vec(model_rdm.to_numpy(), discard_diagonal=True))).correlation)
        ...
        for index, model_rdm in enumerate(dict_models['rdm']):
            for i, sub_rdm in enumerate(target_rdms_trans):
                list_cor_models.append(pearsonr(sym_matrix_to_vec(sub_rdm, discard_diagonal=True), sym_matrix_to_vec(model_rdm.to_numpy(), discard_diagonal=True))[0])
